...
<body onload="init();">
	...
	<audio id="voice" src="<c:url value='/resources/bgm/ch11/11_${scene}_${index}_${flag}.mp3'/>"></audio>
			...
			<div id="background">
				<c:choose>
					<c:when test="${scene eq 1}">
						<jsp:include page="interaction/chapter11/chapter11_1.jsp" />
					</c:when>
					<c:when test="${scene eq 2}">
						<script></script>
						<jsp:include page="interaction/chapter11/chapter11_2.jsp" />
					</c:when>
					<c:when test="${scene eq 3}">
						<jsp:include page="interaction/chapter11/chapter11_3.jsp" />
					</c:when>
				</c:choose>
	...
	<script>
		$(document).ready(function(){
			...
			if(currentPath == "/chapter11part2") {
				$('form').submit();
			}
			else if(currentPath == "/chapter11part3") {
				$('form').attr("action", '/chapter11part2');
				$('form').submit();
			}
		});
	</script>
